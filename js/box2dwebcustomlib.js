...
// Takes world, x, y, width, and height,
// Creates a box at (x,y) with width and height specified,
// returns the body of the kinematic box.
function createKinematicBox(world, x, y, width, height) {
    var fixDef = new b2FixtureDef;
    fixDef.density = 5.0;
    fixDef.friction = 1.0;
    fixDef.restitution = .5;
...
function B2CanvasElement(b2dBody, scale, image, xOffset, yOffset, resize) {
    var that = this;

    that.resize = '30px'; // Amount in pixels to size the image
    that.b2dBody = b2dBody;
    that.scale = scale;
    that.image = image;
    ...
    if (that.xOffset === undefined) {
        that.xOffset = 0;
    }

    if (that.yOffset === undefined) {
        that.yOffset = 0;
    }

    var xOffset = that.xOffset;
    var yOffset = that.yOffset;
    ...
    this.draw = function(context) {
        if (!doNotDraw) {
            ...
            context.drawImage(
                              that.image,
                              ((-that.image.width/2) + (xOffset*(that.image.width))),
                              (-that.image.height/2)
                             );
            context.restore();
        }
...
function drawB2Graphics(canvas, context, listOfB2CanvasElements) {
    context.save();
    context.fillStyle = canvas.b2color;
